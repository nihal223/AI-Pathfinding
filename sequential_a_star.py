...
class AStar(object):
    ...
    def get_path(self, index):
        ...
        for i in range(10000):
            while cell.parent[index][i] is not self.start:
                cell = cell.parent[index][i]
                #print cell.x, cell.y
    ...
    def solve(self,index):
        ...
        f, cell = heapq.heappop(self.opened[index])
        # add cell to closed list so we don't process it twice
        self.closed[index].append(cell)
        # get adjacent cells for cell
        adj_cells = self.get_adjacent_cells(cell)
        for adj in adj_cells:
            if adj.reachable and adj not in self.closed[index]:
                if (adj.f, adj) in self.opened[index]:
                    ...
                    #heapq.heappush(self.opened[index], (adj.f, adj))
                else:
                    self.update_cell(adj, cell, index)
    ...
    #implement the sequential a*
    def seq_astar(self, start, end):
        w2 = 1.7
        self.start = self.get_cell(*start)
        self.end = self.get_cell(*end)
        for i in range(5):
                ...
                if f <= w2*f0:
                    if self.end.g <= f:
                        if self.end.g <=10000:
                            ...
                            #sys.exit(0)
                            return self.get_path(i), self.end.g, len(self.closed[0])+len(self.closed[1])+len(self.closed[2])+len(self.closed[3])+len(self.closed[4])
                    else: 
                        self.solve(i)
                else:
                    if self.end.g <= f0:
                        if self.end.g <=10000:
                            return self.get_path(0), self.end.g, len(self.closed[0])
                    else:
                        self.solve(0)
...
path_map, path_cost, nodes_expanded = AStar_obj.seq_astar(start_map,end_map)
