...
class AStar(object):
    def __init__(self):
        # open list
        self.opened = [[] for i in range(5)]
        heapq.heapify(self.opened[0])
        heapq.heapify(self.opened[1])
        heapq.heapify(self.opened[2])
        heapq.heapify(self.opened[3])
        heapq.heapify(self.opened[4])
        # visited cells list
        self.closed_anchor = []
        self.closed_inad = []
        # grid cells
    ...
    def init_grid(self, width, height, walls, start, end, alg):
    ...
    def get_heuristic(self, cell, index):
    ...
    def update_cell(self, adj, cell, index):
        ...
        #adj.g = cell.g + 1
        adj.h = self.get_heuristic(adj, index)
        #print index
        adj.parent = cell
        if self.alg == 1:
            w = 0
    ...
    def key(self,cell,index):
        
        a = self.get_heuristic(cell,index)
        return cell.g+a
        

    #The main method implements the algorithm itself.    
    def solve(self,index,IorA):
        """Solve maze, find path to ending cell.
        @returns path or None if not found.
        """
        # pop cell from heap queue
        for i in range(5):
            f, cell = heapq.heappop(self.opened[i])
        # add cell to closed list so we don't process it twice
        #print IorA
        if IorA == "inad":
            self.closed_inad.append(cell)
        else:
            self.closed_anchor.append(cell)
        # get adjacent cells for cell
        adj_cells = self.get_adjacent_cells(cell)
        for adj in adj_cells:
            if adj.reachable and adj not in self.closed_anchor:
                if (adj.f, adj) in self.opened[index]:
                    ...
                    if cell_xy[0] in normal and adjacent_xy[0] in normal and direction == "hv" and adj.g > cell.g + 1:
                        self.update_cell(adj, cell, index)
                    ...
                    if cell_xy[0] in hard_highway and adjacent_xy[0] in highway and direction == "d" and adj.g > cell.g + (math.sqrt(2)+math.sqrt(8))/2:
                        self.update_cell(adj, cell, index)
                        '''
                    key_anch = self.key(adj, 0)
                    key_inad = self.key(adj, index)

                    if index == 0:
                        self.opened[index][self.opened[index].index((adj.f,adj))] = (key_anch, adj)
                    else:
                        self.opened[index][self.opened[index].index((adj.f,adj))] = (key_inad, adj)'''
                    ...
                    '''
                    if index == 0:
                        heapq.heappush(self.opened[index], (key_anch, adj))
                    else:
                        heapq.heappush(self.opened[index], (key_inad, adj))
                        '''
                    heapq.heappush(self.opened[0], (key_anch, adj))
                    
                    if adj not in self.closed_inad:
                        for i in range(1,5):
                            if self.key(adj, i) <= 1.7*self.key(adj, 0):
                                #print self.key(adj, i), 1.7*self.key(adj, 0)
                                heapq.heappush(self.opened[i], (key_inad, adj))
                                #print len(self.opened[i])
                                print i
    ...
    #implement the integrated a*
    def int_astar(self, start, end):
        w2 = 1.7
        self.start = self.get_cell(*start)
        self.end = self.get_cell(*end)
        self.start.g =0
        ...
        while len(self.opened[0]):
            print len(self.opened[0])
            f0, cell0 = heapq.nsmallest(1, self.opened[0])[0]
            for i in range(1,5):
                #print len(self.opened[i])
                f, cell = heapq.nsmallest(1, self.opened[i])[0]  
                #print self.end.g,f
                if f <= w2*f0:
                    if self.end.g <= f:
                        if self.end.g <=10000:
                            #sys.exit(0)
                            return self.get_path(), self.end.g, len(self.closed_inad)
                    else: 
                        #print "pop from opened i"
                        self.solve(i, "inad")
                else:
                    if self.end.g <= f0:
                        if self.end.g <=10000:
                            return self.get_path(), self.end.g, len(self.closed_anchor)
                    else:               
                        self.solve(0, "anch")
...
with open('grid_output.txt') as f1:
...
#start_map = ((0,0))
#end_map = ((119,1))
AStar_obj.init_grid(160,120,blocked,start_map,end_map,3)
path_map, path_cost, nodes_expanded = AStar_obj.int_astar(start_map,end_map)
